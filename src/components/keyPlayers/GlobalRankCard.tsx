...
export const GlobalRankCard: React.FC<GlobalRankCardProps> = ({
  ...
  return (
    <div className="relative rounded-3xl bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 border border-amber-500/30 p-6 shadow-2xl overflow-hidden text-slate-100 ring-1 ring-amber-500/20">
      ...
      <div className="relative z-10 mt-5 grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
        {/* Left Avatar & Frame Section */}
        <div className="lg:col-span-5 flex items-center gap-5">
          <div className="relative group cursor-pointer" onClick={onOpenCustomizer}>
            <AvatarFrameRenderer
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            ...
            {/* Rank Tier Pill */}
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-800/90 border border-slate-700/80 text-xs">
              <span className="text-sm">{currentTier.badgeIcon}</span>
              <span className="font-extrabold text-amber-300 tracking-wide uppercase">
                {currentTier.name}
              </span>
              <span className="text-slate-500">•</span>
              <span className="text-slate-300 font-bold text-[11px]">Level {userLevel}</span>
            </div>
            ...
            <div className="space-y-0.5 pt-1 text-xs">
              ...
              <p className="text-amber-400 font-semibold flex items-center gap-1.5 truncate">
        ...
        <div className="lg:col-span-7 space-y-4 bg-slate-950/70 border border-slate-800/90 p-5 rounded-2xl">
          {/* Global Rank & Position Metrics */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="p-3 rounded-xl bg-slate-900/90 border border-slate-800">
              <span className="text-[10px] uppercase font-black tracking-wider text-slate-400">Global Rank</span>
              <div className="text-xl font-black text-amber-400 flex items-center gap-1.5 mt-0.5">
                <Trophy className="w-5 h-5 text-yellow-400" />
                <span>#{userRank}</span>
                <span className="text-xs text-slate-400 font-normal">/ 50k+</span>
              </div>
              <span className="text-[10px] text-emerald-400 font-bold">
                {userRank <= 10 ? '👑 Elite Top 10' : userRank <= 50 ? '🌟 Top 50 Achiever' : '🌍 Top 3.5% Worldwide'}
              </span>
            </div>

            <div className="p-3 rounded-xl bg-slate-900/90 border border-slate-800">
              <span className="text-[10px] uppercase font-black tracking-wider text-slate-400">Total Study XP</span>
              <div className="text-xl font-black text-white flex items-center gap-1.5 mt-0.5">
                <Zap className="w-5 h-5 text-amber-400 fill-amber-400" />
                <span>{userXP.toLocaleString()}</span>
              </div>
              <span className="text-[10px] text-slate-400">Weekly: +{(profile?.streakDays || 1) * 140} XP</span>
            </div>

            <div className="col-span-2 sm:col-span-1 p-3 rounded-xl bg-slate-900/90 border border-slate-800">
              <span className="text-[10px] uppercase font-black tracking-wider text-slate-400">Daily Streak</span>
              <div className="text-xl font-black text-orange-400 flex items-center gap-1.5 mt-0.5">
                <Flame className="w-5 h-5 fill-orange-500" />
                <span>{profile?.streakDays || 1} Days</span>
              </div>
              <span className="text-[10px] text-orange-300 font-bold">Continuous Fire</span>
            </div>
          </div>
          ...
          <div className="space-y-1.5 pt-1">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-slate-300 flex items-center gap-1.5">
                ...
                <span>
                  {userRank === 1
                    ? '👑 You are Sovereign Rank #1 Globally! Maintain the lead!'
